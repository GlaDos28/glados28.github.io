...
class AMDBuilder {
    ...
    nextSprite(texture, time, spriteId = null, nextSpriteId = null) {
        this.__nextSpriteValidation__(texture, time, spriteId, nextSpriteId);
        ...
        const newSprite = new Sprite(this.curFillStateName, texture, time, undefined);
    ...
    formLoop() {
        ...
        this.lastAddedSprite.__setNext__(this.stateFirstSprites[this.curFillStateName]);
    ...
    __nextSpriteValidation__(texture, time, spriteId, nextSpriteId) {

        /* Main */

        this.__ensureNotBuilt__();

        if (typeof texture !== "object" || !texture || !texture.pixiTexture) {
            throw new Error(`Corrupt texture "${texture}". This should be a valid Texture object.`);
        }
